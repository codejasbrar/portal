...
export const loadOrdersByStatus = (status: OrderStatus, page: number) => async (dispatch: Dispatch<object>): Promise<any> => {
  dispatch(loadOrdersRequest());
  try {
    const response = await LabSlipApiService.getOrdersByStatus(status, page);
