...
const SingleSelect = (props: SingleSelectPropsTypes) => {
  ...
  return <div className={`${styles.SelectWrapper} ${props.className ? props.className : ''}`}>
    ...
    <div className={`${styles.Select} ${props.disabled ? styles.SelectDisabled : ''}`}tabIndex={1} onClick={toggleList}>
      ...
      <div className={`${styles.SelectDropdownWrapper} ${opened ? styles.SelectDropdownOpened : ''}`} style={{height: opened ? `${(props.options.length * 36)}px` : 0}}>
        <ul className={styles.SelectDropdown} tabIndex={0}>
          {props.options.map((option: Option) => <li tabIndex={1} className={styles.SelectDropdownItem} onClick={() => selectOption(option)}>{option.name}</li>)}
        </ul>
      </div>
    </ClickOutside>
  </div>
